...
// Start TRNG
	...
	MontR2_params_t MontR2_192_test = { 191,
	0,
	0,	
	...
	if(MontR2_192_test.prec % 32) {
		rw_prec = (MontR2_192_test.prec/32 + 1) * 32;
	} else {
		rw_prec = MontR2_192_test.prec;
	}	
	
	printf("N: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontR2_192_test.n[i]);
	}
	printf("\n\n");	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MONTR2, &MontR2_192_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	...
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontR2_192_test.r2[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontR2 192 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontR2 192 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear r2
	for(i=0; i<rw_prec/32; i++){
		MontR2_192_test.r2[i] = 0;
	}	
	
	MontR2_192_test.sec_calc = 1;	
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontR2 192 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontR2 192 (sec calc) took about %.5f us\n\n", seconds*1000000.0);
