...
// Start TRNG
	...
	ModAdd_params_t ModAdd_191_test = { 191,
	0,
	0,
	...
	if(ModAdd_191_test.prec % 32) {
		rw_prec = (ModAdd_191_test.prec/32 + 1) * 32;
	} else {
		rw_prec = ModAdd_191_test.prec;
	}
	...
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", ModAdd_191_test.a[i]);
	}
	printf("\n\n");	
	...
	printf("B: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", ModAdd_191_test.b[i]);
	}
	printf("\n\n");
	
	printf("N: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", ModAdd_191_test.n[i]);
	}
	printf("\n\n");	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MODADD, &ModAdd_191_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	clock_gettime(CLOCK_MONOTONIC, &tend);

	printf("C = ModAdd(A,B,N): 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", ModAdd_191_test.c[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("ModAdd 191 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("ModAdd 191 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear c
	for(i=0; i<rw_prec/32; i++){
		ModAdd_191_test.c[i] = 0;
	}	
	
	ModAdd_191_test.sec_calc = 1;
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("ModAdd 191 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("ModAdd 191 (sec calc) took about %.5f us\n\n", seconds*1000000.0);	
