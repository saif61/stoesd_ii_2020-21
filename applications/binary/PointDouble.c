...
int main(void)
{
	...
	PointDouble_params_t PointDouble_163_test = { 163,
	0,
	0,
	...
	{ 0x00000000, 0x5973f2ce, 0x9481d196, 0x43a811a8, 0x97599e67, 0x52b62834 }, // xP : 0x525bd3cdda7c989f21652da0de9c7a5d35ae62f1f
	{ 0x00000006, 0x55ebc626, 0xa6197260, 0x18b03f84, 0x730e81a6, 0x19142c94 }, // yP : 0x54972dfe42d888584d8cfce0757f7b0a64801ae68
	...
	if (PointDouble_163_test.prec % 32) {
		rw_prec = (PointDouble_163_test.prec / 32 + 1) * 32;
	}
	else {
		rw_prec = PointDouble_163_test.prec;
	}

	printf("A: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.a[i]);
		//printf("I: %x",i);
	...
	printf("B: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.b[i]);
	}
	printf("\n\n");

	printf("N: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.n[i]);
	}
	printf("\n\n");

	printf("X_P: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.x_p[i]);
	}
	printf("\n\n");

	printf("Y_P: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.y_p[i]);
	}
	printf("\n\n");

	printf("X_Q: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.x_q[i]);
	}
	printf("\n\n");

	printf("Y_Q: 0x");
	for (i = 0; i < rw_prec / 32; i++) {
		printf("%08x", PointDouble_163_test.y_q[i]);
	}
	printf("\n\n");

	ret_val = ioctl(dd, IOCTL_MWMAC_PDOUBLE, &PointDouble_163_test);
	if (ret_val != 0) {
	...
	for (i = 0; i < 128; i++) {	// change: rw_prec/32 --> 128
		printf("%08x", PointDouble_163_test.r_b[i]);
	}
	printf("\n\n");

	printf("r_p: 0x");
	for (i = 0; i < 128; i++) {
		printf("%08x", PointDouble_163_test.r_p[i]);
	}
	printf("\n\n");

	printf("r_a: 0x");
	for (i = 0; i < 128; i++) {
		printf("%08x", PointDouble_163_test.r_a[i]);
	}
	printf("\n\n");

	printf("r_e: 0x");
	for (i = 0; i < 128; i++) {
		printf("%08x", PointDouble_163_test.r_e[i]);
	}
	printf("\n\n");

	printf("r_x: 0x");
	for (i = 0; i < 128; i++) {
		printf("%08x", PointDouble_163_test.r_x[i]);
	}
	printf("\n\n");
