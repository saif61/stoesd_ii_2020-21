...
// Start TRNG
	...
	MontMult_params_t MontMult_191_test = { 191,
	0,
	0,
	...
	if(MontMult_191_test.prec % 32) {
		rw_prec = (MontMult_191_test.prec/32 + 1) * 32;
	} else {
		rw_prec = MontMult_191_test.prec;
	}
	...
	while (i < 1) {
		...
		if(ret_val == 0) {
			trng_val &= 0x7FFFFFFF;
			MontMult_191_test.a[0] = trng_val;
			i++;
	...
	while (i < rw_prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontMult_191_test.a[i] = trng_val;
			i++;
		} else if (ret_val == -EAGAIN) {
			printf("TRNG FIFO empty\n");
		} else {
	...
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontMult_191_test.a[i]);
	}
	printf("\n\n");
	...
	while (i < 1) {
		...
		if(ret_val == 0) {
			trng_val &= 0x7FFFFFFF;
			MontMult_191_test.b[0] = trng_val;
			i++;
		} else if (ret_val == -EAGAIN) {
			printf("TRNG FIFO empty\n");
		} else {
	...
	while (i < rw_prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontMult_191_test.b[i] = trng_val;
			i++;
		} else if (ret_val == -EAGAIN) {
			printf("TRNG FIFO empty\n");
		} else {
	...
	printf("B: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontMult_191_test.b[i]);
	}
	printf("\n\n");	

	printf("N: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontMult_191_test.n[i]);
	}
	printf("\n\n");	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MONTMULT, &MontMult_191_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	clock_gettime(CLOCK_MONOTONIC, &tend);

	printf("C = MontMult(A,B,N): 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", MontMult_191_test.c[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontMult 191 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontMult 191 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear c
	for(i=0; i<rw_prec/32; i++){
		MontMult_191_test.c[i] = 0;
	}	
	
	MontMult_191_test.sec_calc = 1;		
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontMult 191 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontMult 191 (sec calc) took about %.5f us\n\n", seconds*1000000.0);	
