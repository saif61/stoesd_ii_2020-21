...
// Start TRNG
	...
	CopyV2H_params_t CopyV2H_191_test = { 191,
	0,
	0,	
	{  },
	{  },
	...
	if(CopyV2H_191_test.prec % 32) {
		rw_prec = (CopyV2H_191_test.prec/32 + 1) * 32;
	} else {
		rw_prec = CopyV2H_191_test.prec;
	}
	...
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", CopyV2H_191_test.a[i]);
	}
	printf("\n\n");

	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_COPYV2H, &CopyV2H_191_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	...
	printf("A-Copy: 0x");
	for(i=0; i<rw_prec/32; i++){
		printf("%08x", CopyV2H_191_test.acopy[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("CopyV2H 191 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("CopyV2H 191 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear acopy
	for(i=0; i<rw_prec/32; i++){
		CopyV2H_191_test.acopy[i] = 0;
	}	
	
	CopyV2H_191_test.sec_calc = 1;	
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("CopyV2H 191 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("CopyV2H 191 (sec calc) took about %.5f us\n\n", seconds*1000000.0);	
