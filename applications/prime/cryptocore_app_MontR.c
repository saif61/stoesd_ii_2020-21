...
// Start TRNG
	...
	MontR_params_t MontR_512_test = { 512,
	1,
	0,
	...
	printf("N: 0x");
	for(i=0; i<MontR_512_test.prec/32; i++){
		printf("%08x", MontR_512_test.n[i]);
	}
	printf("\n\n");	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MONTR, &MontR_512_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	...
	printf("R = MontR(N): 0x");
	for(i=0; i<MontR_512_test.prec/32; i++){
		printf("%08x", MontR_512_test.r[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontR 512 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontR 512 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear r
	for(i=0; i<MontR_512_test.prec/32; i++){
		MontR_512_test.r[i] = 0;
	}	
	
	MontR_512_test.sec_calc = 1;	
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontR 512 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontR 512 (sec calc) took about %.5f us\n\n", seconds*1000000.0);
