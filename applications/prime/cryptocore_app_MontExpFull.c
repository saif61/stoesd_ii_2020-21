...
// Start TRNG
	...
	i = 0;
	while (i < MontExp_512_test.prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontExp_512_test.b[i] = trng_val;
			i++;
	...
	for(i=0; i<MontExp_512_test.prec/32; i++){
		printf("%08x", MontExp_512_test.b[i]);
	}
	printf("\n\n");
...
*/
	...
	printf("E: 0x");
	for(i=0; i<MontExp_512_test.prec/32; i++){
		printf("%08x", MontExp_512_test.e[i]);
	}
	printf("\n\n");	

	printf("N: 0x");
	for(i=0; i<MontExp_512_test.prec/32; i++){
		printf("%08x", MontExp_512_test.n[i]);
	}
	printf("\n\n");	
	...
	MontExp_512_test.sec_calc = 0;	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MONTEXP_FULL, &MontExp_512_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	clock_gettime(CLOCK_MONOTONIC, &tend);
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontExpFull 512 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontExpFull 512 took about %.5f us\n\n", seconds*1000000.0);	
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontExpFull 512 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontExpFull 512 (sec calc) took about %.5f us\n\n", seconds*1000000.0);
