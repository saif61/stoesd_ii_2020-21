...
// Start TRNG
	...
	ModAdd_params_t ModAdd_512_test = { 512,
	1,
	0,
	...
	i = 0;
	while (i < ModAdd_512_test.prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			ModAdd_512_test.a[i] = trng_val;
			i++;
	...
	printf("A: 0x");
	for(i=0; i<ModAdd_512_test.prec/32; i++){
		printf("%08x", ModAdd_512_test.a[i]);
	}
	printf("\n\n");	
	...
	printf("B: 0x");
	for(i=0; i<ModAdd_512_test.prec/32; i++){
		printf("%08x", ModAdd_512_test.b[i]);
	}
	printf("\n\n");
	
	printf("N: 0x");
	for(i=0; i<ModAdd_512_test.prec/32; i++){
		printf("%08x", ModAdd_512_test.n[i]);
	}
	printf("\n\n");	
	
	clock_gettime(CLOCK_MONOTONIC, &tstart);
	ret_val = ioctl(dd, IOCTL_MWMAC_MODADD, &ModAdd_512_test);
	if(ret_val != 0) {
		printf("Error occured\n");
	}
	...
	for(i=0; i<ModAdd_512_test.prec/32; i++){
		printf("%08x", ModAdd_512_test.c[i]);
	}
	printf("\n\n");

	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("ModAdd 512 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("ModAdd 512 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear c
	for(i=0; i<ModAdd_512_test.prec/32; i++){
		ModAdd_512_test.c[i] = 0;
	}	
	
	ModAdd_512_test.sec_calc = 1;
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("ModAdd 512 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("ModAdd 512 (sec calc) took about %.5f us\n\n", seconds*1000000.0);	
