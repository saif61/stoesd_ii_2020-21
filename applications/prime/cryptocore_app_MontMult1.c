...
// Start TRNG
	...
	i = 0;
	while (i < MontMult1_512_test.prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			MontMult1_512_test.b[i] = trng_val;
			i++;
	...
	printf("N: 0x");
	for(i=0; i<MontMult1_512_test.prec/32; i++){
		printf("%08x", MontMult1_512_test.n[i]);
	}
	printf("\n\n");	
	...
	if (seconds*1000000.0 > 1000.0)
		printf("MontMult1 512 took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontMult1 512 took about %.5f us\n\n", seconds*1000000.0);	

	// Clear c
	for(i=0; i<MontMult1_512_test.prec/32; i++){
		MontMult1_512_test.c[i] = 0;
	}	
	
	MontMult1_512_test.sec_calc = 1;		
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("MontMult1 512 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("MontMult1 512 (sec calc) took about %.5f us\n\n", seconds*1000000.0);
