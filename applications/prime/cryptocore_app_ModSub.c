...
// Start TRNG
	...
	i = 0;
	while (i < ModSub_512_test.prec/32) {
		ret_val = ioctl(dd, IOCTL_READ_TRNG_FIFO, &trng_val);
		if(ret_val == 0) {
			ModSub_512_test.a[i] = trng_val;
			i++;
	...
	for(i=0; i<ModSub_512_test.prec/32; i++){
		printf("%08x", ModSub_512_test.a[i]);
	}
	printf("\n\n");	
	...
	printf("B: 0x");
	for(i=0; i<ModSub_512_test.prec/32; i++){
		printf("%08x", ModSub_512_test.b[i]);
	}
	printf("\n\n");
	
	printf("N: 0x");
	for(i=0; i<ModSub_512_test.prec/32; i++){
		printf("%08x", ModSub_512_test.n[i]);
	}
	printf("\n\n");	
	...
	seconds = ((double)tend.tv_sec + 1.0e-9*tend.tv_nsec) - ((double)tstart.tv_sec + 1.0e-9*tstart.tv_nsec);
	if (seconds*1000000.0 > 1000.0)
		printf("ModSub 512 (sec calc) took about %.5f ms\n\n", seconds*1000.0);
	else 
		printf("ModSub 512 (sec calc) took about %.5f us\n\n", seconds*1000000.0);
